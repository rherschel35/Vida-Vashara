"""
Vida Vashara - founder of House Vashara, the healer who never wanted to be
one, and the ghost who cared for Mordy Velmora at the very end of his life.
Entry point: wires up the client and loads cogs. She only ever speaks in
response to someone, and she never talks to the other ghosts - not even
Cassy, whom she has never once addressed directly, however fond she is
of her from a distance.
"""
...
bot = commands.Bot(command_prefix="!vashara-unused-", intents=intents, help_command=None)
...
@bot.event
async def on_ready():
    log.info("Vida has arrived. Logged in as %s (id=%s)", bot.user, bot.user.id)
